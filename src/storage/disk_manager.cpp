...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    off_t write_offset = page_no * PAGE_SIZE;
    off_t ret = lseek(fd, write_offset, SEEK_SET);
    if (ret == -1) {
        throw UnixError();
    }
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    off_t read_offset = page_no * PAGE_SIZE;
    off_t ret = lseek(fd, read_offset, SEEK_SET);
    if (ret == -1) {
        throw UnixError();
    }
